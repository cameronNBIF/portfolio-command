...
**The schedule sits on the Finance tab, not the Policies tab** — a fifth surface after Transactions, Valuation Marks, FMV Review and LP Activity. The Policies tab holds the two policy sections and nothing else. Cameron's call on the day, and it draws the line the tab was created to draw: configuration is a rule, and the schedule is the work the rule produces.
