...
**Disposition:** Direction confirmed. Build the push at A13. **Take the frozen snapshot immediately** (see roadmap phase F0). Raise the ADR now, because this reverses a rule ADR-009 states categorically.
**Size:** M at A13 · S for the snapshot · **Schema:** Minor (a snapshot table, plus sync-state tracking later)
